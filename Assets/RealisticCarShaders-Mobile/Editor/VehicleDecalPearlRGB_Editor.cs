...
namespace SkrilStudio
{
    ...
    {
        private const string ecsFolderGUID = "cc35dd109029b5b4fad9811ad43ccdf3";
        private static readonly string _logoImagePath = AssetDatabase.GUIDToAssetPath(ecsFolderGUID) + "/Editor/logoPearlRGB.png";
        ...
        MaterialProperty _MetalicnessIntensity;
        MaterialProperty _MetalicnessScale;
        ...
        private bool firstApply = true,
             BodyUVFold, DecalsUVFold, PearlUVFold, DiffuseBump, PearlBump, HighLightDecal, DiffuseReflect;

        // custom logo
        readonly Texture texLogo = AssetDatabase.LoadAssetAtPath<Texture>(_logoImagePath);
        ...
        void GetValues()
        {
            ...
            _MetalicnessIntensity = FindProperty("_MetalicnessIntensity", materialProperties);
            _MetalicnessScale = FindProperty("_MetalicnessScale", materialProperties);
            // enum
            ...
            if (_material.IsKeywordEnabled("Bumped_Pearl"))
                ...
                PearlBump = false;
            if (_material.IsKeywordEnabled("DecalHighlightOn"))
                HighLightDecal = true;
            else
                HighLightDecal = false;
            // diffuse reflection
            if (_material.IsKeywordEnabled("DiffuseReflectOff"))
                DiffuseReflect = true;
            else
                DiffuseReflect = false;
        }
        ...
        void ShowProperties()
        {
            ...
            if (reflectionType == ReflectionType.TurnedOff)
            {
                ...
                DiffuseReflect = EditorGUILayout.Toggle("Diffuse Reflection", DiffuseReflect);
            }
            ...
            materialEditor.ShaderProperty(_MetalicnessIntensity, "Metalicness Intensity");
            materialEditor.ShaderProperty(_MetalicnessScale, "Metalicness Scale");
            ...
            if (DiffuseReflect)
            {
                _material.EnableKeyword("DiffuseReflectOff");
                _material.DisableKeyword("DiffuseReflectOn");
            }
            else
            {
                _material.EnableKeyword("DiffuseReflectOn");
                _material.DisableKeyword("DiffuseReflectOff");
            }
            ...
            HighLightDecal = EditorGUILayout.Toggle("Highligh Decal", HighLightDecal);
            ...
            if (HighLightDecal)
                _material.EnableKeyword("DecalHighlightOn");
            else
                _material.DisableKeyword("DecalHighlightOn");
