...
public class RaceProgressTracker : MonoBehaviour
{
    ...
    //        //if (expectedCheckpointIndex == 0)
    //        if (expectedCheckpointIndex == _checkpoints.Length - 1)
    //        {
    //            racer.CompleteLap();
    ...
    //                if (ReferenceEquals(racer, _playerRacer))
    //                {
    //                    EndRace(racer);
    //                }
    //            }
    ...
    private void EndRace(Racer finishingRacer)
    {
        ...
        for (int i = 0; i < _racers.Length; i++)
        {
            ...
            Debug.Log($"Место: {i + 1} | RacerID: {r.RacerId} | Круги: {r.LapsCompleted}");
        }
